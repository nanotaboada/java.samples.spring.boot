...
public class PlayersService {
     ...
     * <p>
     * Converts the PlayerDTO to a Player entity, persists it (triggering UUID generation via {@code @PrePersist}),
     * and returns the saved player. The cache is evicted to maintain consistency.
     * </p>
     * <p>
    ...
    /**
     * Retrieves a player by their surrogate UUID (admin/internal use only).
     * <p>
     * Uses caching to improve performance. Null results are not cached.
     * </p>
     *
     * @param id the UUID surrogate key (must not be null)
     ...
     * <p>
     * Performs a full update (PUT semantics) of the player entity. If the player exists,
     * it updates all fields and evicts the cache. If the player doesn't exist, returns false.
     * </p>
     *
    ...
    public boolean update(Integer squadNumber, PlayerDTO playerDTO) {
        ...
        if (squadNumber != null && playersRepository.existsById(squadNumber)) {
            Player player = mapFrom(playerDTO);
     ...
     * <p>
     * Removes the player from the database and evicts the cache. If the player doesn't exist,
     * returns false without making changes.
     * </p>
     *
    ...
    public boolean deleteBySquadNumber(Integer squadNumber) {
        ...
        if (playersRepository.existsById(squadNumber)) {
            playersRepository.deleteById(squadNumber);
